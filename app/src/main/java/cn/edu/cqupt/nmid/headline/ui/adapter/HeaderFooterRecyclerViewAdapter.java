...
public abstract class HeaderFooterRecyclerViewAdapter
  ...
  /**
   * Gets the header item count. This method can be called several times, so it should not calculate
   * the count every time.
   *
  ...
  /**
   * Gets the footer item count. This method can be called several times, so it should not calculate
   * the count every time.
   *
  ...
  /**
   * This method works exactly the same as {@link #onCreateViewHolder(android.view.ViewGroup, int)},
