...
public class Controller {
	...
	public Controller(Context context) {
		dataBaseHelper = FeedsDataBaseHelper
				.getInstance(context, "headline.db", 1);
	}
	...
	public void likeIt(int id, int category, boolean isCcollect){
			int i=isCcollect ? 1 : 0;
			dataBaseHelper.upData(id, Constant.TYPE[category], i);
	}

	/*
	*得到收藏列表
	*/
	public LinkedList<NewsBean> getFavoriteList() {
		LinkedList<NewsBean> tempList = new LinkedList<NewsBean>();
		Cursor cursor = null;
		cursor = dataBaseHelper
				.getData("select * from college where iscollect=1 order by _id desc");
		while (cursor.moveToNext()) {
			tempList.add(getBean(cursor));
		}
		cursor.close();
		cursor = dataBaseHelper
				.getData("select * from scientific where iscollect=1 order by _id desc");
		while (cursor.moveToNext()) {
			tempList.add(getBean(cursor));
		}
		cursor.close();
		cursor = dataBaseHelper
				.getData("select * from youth where iscollect=1 order by _id desc");
		while (cursor.moveToNext()) {
			tempList.add(getBean(cursor));
		}
		cursor.close();
		cursor = dataBaseHelper
				.getData("select * from classmate where iscollect=1 order by _id desc");
		while (cursor.moveToNext()) {
			tempList.add(getBean(cursor));
		}
		cursor.close();
	...
	public LinkedList<NewsBean> getInitData(LinkedList<NewsBean> listBeans, int category,
		...
		Cursor cursor = dataBaseHelper
				.getData("select * from " + Constant.TYPE[category]
		...
		if (cursor.getCount() == limit) {
			...
			HeadJson bean = getHeadJson(id, category,
					limit - cursor.getCount(), "freshnews");
			if (bean.getStatus() == 0)
                return listBeans;
	 ...
	 * 
	 */
	public void getOldData(LinkedList<NewsBean> listBeans, int category,
			int limit) {
		...
		if (cursor.getCount() == 0) {
			...
			while (cursor.moveToNext()) {
				if (cursor.getInt(0) == id) {
					...
					HeadJson bean = getHeadJson(id + 1, category, num,
							"oldnews");
					if (bean.getData() == null) {
						return;
					}
